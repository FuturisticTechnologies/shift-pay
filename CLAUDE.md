...
- **Do not use the `reyantech` account or the `shiftpay-reyantech` profile.** That account is being retired: it must not appear as a shift owner, manager, approver, or in `sys_created_by`/`sys_updated_by` on anything new. `sys_created_by` is immutable over REST — ServiceNow silently ignores attempts to set it — so get the profile right *before* the first insert. Historic records were corrected on 15 Aug 2026: `One Time Scripts/reattribute-created-by.js` re-attributed all 67 ShiftPay artifacts to `admin` via `autoSysFields(false)`, which is the only lever and has no REST equivalent. Re-running it now reports 0. It deliberately leaves non-ShiftPay records alone, including ones the retired account merely touched.
