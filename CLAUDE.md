...
- `Manager Approval UI Widget/` — in progress. Team approval queue: pending timesheets with approve/reject, a review drill-in, per-day correction with a mandatory reason, and per-shift-type cost. Queue, review panel and per-day correction are built and deployed; bulk actions, the history tab, and ACLs are not. It reimplements no shift or pay rule — everything comes from the three Script Includes.
