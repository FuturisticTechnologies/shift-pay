...
**CO entitlement flow** (server `handleAction` → `ShiftPayEntitlements`) — gated by the `enable_co_entitlement` option (default `true`; when `false` every entitlement branch is skipped and CO/compound-OC act as ordinary shifts): logging a `grants_co` shift inserts an entitlement row with a window of 7 *weekdays* (`_nthWeekdayAfter`). The `consumes_co` shift is only selectable on a weekday covered by an unconsumed entitlement window (`computeAllowedShifts` → `hasUnconsumedFor`). Saving it consumes the earliest eligible entitlement; clearing/changing an entry releases or deletes the entitlement, and the server **blocks** removing an OC entry that a CO still depends on. `consumes_co`/`grants_co` shifts are deliberately rejected in `bulkSave` (`blocksBulk`).

A day change is a **three-phase transaction** that must bracket the day write, because the entitlement decision has to be made before the row exists and recorded after: `releasePrevious(date, prevShift, newShift)` → *write the day* → `commitForNew(...)`, with `reserveForNew(...)` in between claiming the entitlement a CO needs. Pass `newShift` as null to `releasePrevious` when clearing — that selects the "Cannot clear" wording rather than "Cannot change this entry".

**Allowed-shifts are computed server-side, enforced client-side.** `data.allowedShifts` is `{ 'YYYY-MM-DD': [sys_id,...] }` from `ShiftPayCalendarRules.allowedForMonth`; the client's `c.isAllowed` filters dropdowns. Weekend/holiday detection uses a `cmn_schedule` whose sys_id is in system property `x_shiftpay.holiday_schedule` (read via `cmn_schedule_span`). Client-side filtering is convenience only — any server action that writes a day must gate on `isAllowedOn` itself, because `c.server.get()` is callable from the browser with any payload.
...
**Approval state lives on the monthly-timesheet row**, not a separate table: `status` (submitted/approved/rejected), `approver`, `actioned_on`, `manager_comment`. A month is locked when a row exists **and** `status != 'rejected'` — rejection reopens it for editing, and `submitMonth` reuses the existing row on resubmission rather than inserting a second one.

**Bulk approve/reject loops `actionOne`** rather than issuing a bulk query, so every authorisation and state guard applies per row. `actionOne` therefore *returns* its refusal instead of raising it: the single-row path shows it immediately, the bulk path collects the distinct reasons into one summary. The client's selection is derived from the **visible** rows, so a row filtered out by tab or search can never be swept into a bulk action.

**The History tab** (`loadHistory`) is scoped by `approver = me`, not by the reportee list — a decision stays yours when someone changes team, and the query cannot return anyone else's. It is read-only and spans every month, so the month picker and stat strip are hidden on it. Note it shows only decisions that **still stand**: `submitMonth` clears `approver`/`actioned_on`/`manager_comment` on resubmission, so a rejection the employee has since fixed disappears from the log. That is the accepted cost of keeping approval state on the timesheet row.
...
- **`cnit` has no `delete` verb** ("use the UI"). Deleting has to go through the platform UI; `scratchpad/si-delete.js` drives it with Playwright. The confirmation dialog renders inside the `gsft_main` iframe, not the top frame — searching the top frame finds zero buttons and silently does nothing, leaving the record alive.
...
**Running server-side scripts: use a Fix Script, not Scripts - Background.** A `sys_script_fix` record carries its scope as a *field*, so what it will run as can be read back and asserted before anything executes. A background script silently inherits whatever scope the session is in — and running a global-table script in the app scope makes the platform auto-grant the app cross-scope read privileges (`sys_scope_privilege` rows) for every platform table touched, which then has to be cleaned up by hand. `gs.print` is also blocked inside a scoped app; guard it and pair it with `gs.info` so the output survives in the syslog either way.
...
**One-time sweeps must be confined per table, and the key differs.** `sys_dictionary`/`sys_choice`/`sys_db_object` key on `name STARTSWITH x_1995110_shift_0` — a `sys_scope` filter there is wrong, because 36 of the 53 ShiftPay dictionary rows (the auto-generated `sys_*` columns) carry an *empty* scope and would be silently skipped. `sys_script_include`/`sp_widget` have no table name to key on, so they must use `sys_scope`. Matching on the user alone is a blanket instance sweep: the first re-attribution dry run offered to rewrite two CAF Compliance widgets and an unrelated Script Include, one of them authored by another person and merely *touched* by the retired account.

**…but do not enumerate the classes.** The corollary bit once, and the script that got it wrong has since been deleted rather than left around to copy from: the first sweep listed seven tables by hand, reported 0 on a re-run, and looked finished while 36 rows in nine *other* metadata classes were still stamped `reyantech` — `sys_documentation` (field labels), `sys_app_module`, `sys_choice_set`, `sys_scope_privilege`, `sys_script_fix`, `sys_ui_list`, `sys_ui_form`, `sp_instance`, `ua_table_licensing_config`. An application file is any row on a table extending **`sys_metadata`**, so an app-wide sweep queries that base table with `sys_scope=<app>` and lets the scope confine it — new classes are then covered without editing the script. `One Time Scripts/reattribute-metadata-by-scope.js` is that sweep, and it keeps the two keys above as extra phases for what `sys_metadata` cannot express (the empty-scope dictionary rows, the app's own data tables). Collect the rows first, then write each through a GlideRecord on its **real** `sys_class_name`, not through `sys_metadata`. Exclude `sys_class_name=sys_metadata_delete`: those are tombstones for records already gone, and rewriting who deleted what is falsifying history — one such row (`New section 1`, deleted by `reyantech`) is deliberately left alone.
...
- **Do not use the `reyantech` account or the `shiftpay-reyantech` profile.** That account is being retired: it must not appear as a shift owner, manager, approver, or in `sys_created_by`/`sys_updated_by` on anything new. `sys_created_by` is immutable over REST — ServiceNow silently ignores attempts to set it — so get the profile right *before* the first insert. Historic records were corrected in two passes, both via `autoSysFields(false)` — the only lever, with no REST equivalent — and both deliberately leaving non-ShiftPay records alone, including ones the retired account merely touched:
  - 15 Aug 2026 — 67 artifacts across seven hand-listed tables. That script has been deleted: it was spent, and the second pass covers everything it did.
  - 19 Aug 2026, `One Time Scripts/reattribute-metadata-by-scope.js` — the 36 rows that list missed, swept from the `sys_metadata` base table instead. See the enumeration warning under *Deploying to the instance*. Kept because it is the re-runnable check, not because there is work left: it reports 0.

  Both now report 0. The check that catches a miss is `sys_metadata` with `sys_scope=bbd97549938183507f08f2a0ed03d60b^sys_class_name!=sys_metadata_delete^sys_updated_by!=admin^ORsys_updated_by=NULL` — it returns 0 rows, and any new stamp from a wrong profile shows up there whatever class it lands on.
