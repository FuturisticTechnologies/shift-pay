(function () {
  ...
  // Which scope the page opens on when the request names none. This is a
  // preference, not a permission: resolveScope still checks the requested scope
  // against data.scopes, so a user without ORG_ROLE asking for 'org' — or
  // landing here with 'org' as the default — still falls back to their own data.
  var DEFAULT_SCOPE = options.default_scope || 'org';
  ...
  function resolveScope(requested) {
    var want = trim(requested) || DEFAULT_SCOPE;
